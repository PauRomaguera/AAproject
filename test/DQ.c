...
void directInvert(REAL *A, REAL *inverse, int n) {
    // Simplified direct inversion, e.g., Gaussian elimination for small matrices.
    // Implemented similarly to your original code but only for small matrices.
}

void multiplyMatrices(REAL *A, REAL *B, REAL *result, int n) {
    // Standard matrix multiplication for size n
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < n; j++) {
            result[i * n + j] = 0.0;
            for (int k = 0; k < n; k++) {
